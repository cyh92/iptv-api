...
open_proxy = False
# 开启查询请求，数据来源于网络（仅针对酒店源与组播源）; 可选值: True, False | Enable query request, data comes from the network (only for hotel source 和 multicast source); Optional values: True, False
open_request = False
...
open_url_info = False
# 开启使用本地缓存数据，适用于查询请求失败场景（仅针对酒店源与组播源）; 可选值: True, False | Enable to use local cached data, suitable for query request failure scenarios (only for hotel source 和 multicast source); Optional values: True, False
open_use_cache = True
...
origin_type_prefer =subscribe,multicast,hotel
